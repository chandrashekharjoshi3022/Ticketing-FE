...
const normalizeTicket = (t, idx = 0) => ({
  ...
  // --- new fields for assignment/executive support:
  assigned_to: t.assigned_to ?? null,
  assignee: t.assignee
    ? {
        id: t.assignee.user_id ?? t.assignee.id,
  ...
  // ADD THIS: Include is_other_issue field
  is_other_issue: t.is_other_issue ?? false
});
...
export const fetchTickets = createAsyncThunk('tickets/fetchAll', async (_, thunkAPI) => {
  try {
    ...
    const userRole = state.auth.user?.role_name ?? state.auth.user?.role ?? 'user';
    const res = await TicketService.getTickets(userRole);
...
export const replyToTicket = createAsyncThunk('tickets/replyToTicket', async ({ ticketId, formData }, { rejectWithValue }) => {
  try {
    ...
    builder
      ...
      .addCase(fetchTicketDetails.fulfilled, (state, action) => {
        ...
        state.ticketDetails = {
          ...
          // ADD THIS: Include is_other_issue field
          is_other_issue: d.is_other_issue ?? false
        };
      })
