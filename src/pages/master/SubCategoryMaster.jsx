...
import {
  Box, Grid, MenuItem, IconButton, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions
} from '@mui/material';
...
export default function SubCategoryMaster() {
  ...
  return (
    <MainCard title={<Box display="flex" justifyContent="space-between" alignItems="center"><span>Sub Categories</span><PlusButton label="New" onClick={() => setEditing(null)} /></Box>}>
      <Box sx={{ height: '80dvh', overflowY: 'auto' }}>
        {isLoading ? <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 200 }}><LoaderLogo /></Box> : (
          <>
            <Formik enableReinitialize initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit} key={editing ? editing.subcategory_id : 'new'}>
              {({ resetForm }) => (
                <Form>
                  <Grid container spacing={2} alignItems="center" sx={{ mb: 1 }}>
                    ...
                    <Grid item xs={12} sm={12} display="flex" gap={1} justifyContent="flex-end">
                      ...
                      <CustomRefreshBtn onClick={() => { resetForm(); setEditing(null); dispatch(fetchSubCategories()); dispatch(fetchCategories()); dispatch(clearSubcategoryError()); }}>Refresh</CustomRefreshBtn>
                    </Grid>
                  </Grid>
                </Form>
              )}
        ...
        <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
          ...
          <DialogContent><DialogContentText>Are you sure you want to delete this subcategory?</DialogContentText></DialogContent>
          <DialogActions>
